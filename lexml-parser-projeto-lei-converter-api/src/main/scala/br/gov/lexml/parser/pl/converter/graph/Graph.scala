package br.gov.lexml.parser.pl.converter.graph
...
class Graph[E, N](var value: N = null.asInstanceOf[N]) {
  ...
  override def toString: String = "Graph(" + value + ")"
   ...
   def dijkstra(from: N, to: N)(distance : (N,E,N) => Double): IndexedSeq[(N,E,N)] = {
    var distances = nodesByDepth.map((_, Double.PositiveInfinity)).toMap + (from -> 0.0)
    var spt: Map[N, Edge[E,N]] = Map()

    while (!distances.isEmpty) {
      //println(s"distances=${distances}, spt=${spt}")
      ...
      val (v, dv) = distances.minBy(_._2)
      //println(s"v=${v}, dv=${dv}")
      distances = distances - v

      val vv = hop(v).get
      for (e <- vv.outEdges) {
        val d = distance(e.source.value,e.value,e.target.value)        
        val u = e.target.value
        //println(s"... u=${u},d=${d}")
        if (distances.contains(u) && distances(u) > dv + d) {
          distances = distances + (u -> (dv + d))
          spt = spt + (u -> e)
        }
      }
    }
    ...
    var step = to
    var path = IndexedSeq.newBuilder[(N,E,N)]
    //println("final spt:")
    //spt.foreach({case (k,v) => println(s"  {$k}: ${v.source.value}")})  
    
    while (spt.contains(step)) {
      //println(s"step=${step} path=${path.result().map(x => (x._1,x._3))}")
      val e = spt(step)
      step = e.source.value      
      path += ((e.source.value,e.value,e.target.value)) 
    }

    val res = path.result().reverse
    //println(s"resulting path: ${res.map(x => (x._1,x._3))}")
    res
  }
...
class WeightedGraph[N](n: N) extends Graph[Double, N](n) {
   ...
   * Space - O()
   */
  def dijkstra1(from: N, to: N): IndexedSeq[N] = dijkstra(from,to){ case (_,d,_) => d }.map(_._1) :+ to
