...
class FECmdLineOptionParser extends scopt.OptionParser[CmdLineOpts]("parser"):
  ...
  private def optOverridesRegex(suffix: String, desc: String)(
    ...
    opt[String](s"prof-regex-$suffix")
      .valueName("regex%regex%...")
      .action {
        optOverrides { case (rl, o) =>
          f(o, stringToRegexList(rl))
        }
      }
      .text(
  ...
  cmd("parse")
    .action { (_, opts) => opts.copy(cmd = CmdParse()) }
    .text("Faz o parse de um documento completo")
    .children(
      opt[String]('m', "mime-type")
        .text(
          "tipo MIME do arquivo de entrada, ex.: text/plain (default)," +
            " application/vnd.openxmlformats-officedocument.wordprocessingml.document, " +
            "... "
        )
        .action {
          cmdParse { case (mt, cmd) =>
      ...
      opt[File]("write-errors-to-file")
        .text(
          "gravar erros em arquivo de saída. Se omitido, será usado a saída de erro padrão"
        )
        .action {
          cmdParse { case (f, cmd) =>
      ...
      opt[Unit]("info")
        ...
        .text(
          "Mostra os parâmetros usados no parse e sai sem executar o parse"
        ),
      opt[String]('t', "tipo-norma")
        .action {
          cmdParse { case (n, cmd) =>
            cmd changeMetadado {
      ...
      opt[String]('l', "localidade")
        .action {
          cmdParse { case (n, cmd) =>
            ...
            cmd.changeOverrides(
              _.copy(overrideUrnFragLocalidade = Some(Some(n)))
            )
          }
        }
        .text("localidade, na sintaxe usada na URN do LexML, eg. 'br'."),
      opt[Int]('n', "numero")
        .valueName("número da norma")
        .action {
          cmdParse { case (n, cmd) =>
            cmd changeMetadado {
              _ changeId {
      ...
      opt[Int]("complemento")
        .valueName("complemento do número da norma")
        .action {
          cmdParse { case (n, cmd) =>
            cmd changeMetadado {
              _ changeId {
      ...
      opt[Int]("ano")
        ...
        .text(
          "ano da publicação da norma (quando o mês e dia são desconhecidos)"
        ),
      opt[String]("data")
        .action {
          cmdParse { case (n, cmd) =>
      ...
      opt[String]("timestamp-evento")
        ...
        .text(
          "data e horário do evento que marca a versão da norma (AAAA-MM-DDtHH:MM)"
        ),
      optOverridesRegex(
        "local-data",
        "a expressão do local e data antes da assinatura"
      ) { case (o, rl) =>
        o copy (overrideRegexLocalData = rl)
      },
      ...
      optOverridesRegex(
        "legislacao-citada",
        "o início da seção de Legislação Citada"
      ) { case (o, rl) =>
        o copy (overrideRegexLegislacaoCitada = rl)
      },
      optOverridesRegex("assinatura", "o iníco da assinatura") { case (o, rl) =>
        o copy (overrideRegexAssinatura = rl)
      },
      optOverridesRegex("epigrafe", "a epígrafe") { case (o, rl) =>
        o copy (overrideRegexEpigrafe1 = rl)
      },
      optOverridesRegex("epigrafe-continuacao", "a epígrafe") { case (o, rl) =>
        o copy (overrideRegexEpigrafe = rl)
      },
      optOverridesRegex("pos-epigrafe", "o texto entre a epígrafe e a ementa") {
        case (o, rl) => o copy (overrideRegexPosEpigrafe = rl)
      },
      optOverridesRegex("preambulo", "o pré-âmbulo") { case (o, rl) =>
        o copy (overrideRegexPreambulo = rl)
      },
      opt[Unit]("prof-epigrafe-obrigatoria")
        .action {
          optOverrides { case (_, o) =>
            o.copy(overrideEpigrafeObrigatoria = Some(true))
          }
        }
        .text("a epígrafe precisa estar presente"),
      opt[Unit]("prof-epigrafe-opcional")
        .action {
          optOverrides { case (_, o) =>
      ...
      opt[Unit]("prof-pre-epigrafe-permitida")
        .action {
          optOverrides { case (_, o) =>
      ...
      opt[Unit]("prof-epigrafe-nao-permitida")
        .action {
          optOverrides { case (_, o) =>
      ...
      opt[String]("prof-epigrafe-head")
        .action {
          optOverrides { case (x, o) =>
            o.copy(overrideEpigrafeHead = Some(x))
          }
        }
        .text("texto que precede o número na epígrafe"),
      opt[String]("prof-epigrafe-tail")
        .action {
          optOverrides { case (x, o) =>
      ...
      opt[File]("linker")
        ...
        .text(
          "caminho para o executável do linker. Se omitido o linker não será usado"
        )
    )
  cmd("dumpProfiles")
    .action { (_, opts) =>
  ...
  cmd("parseArticulacao")
    .text(
      "faz o parse só da articulacao. A entrada deve ser um documento xml com apenas parágrafos XHTML abaixo do elemento raiz"
    )
    .action { (_, opts) =>
    ...
    .children(
      opt[File]('i', "input")
        .text("arquivo de entrada. Se omitido, será usado a entrada padrão")
        .action {
          cmdParseArticulacao { case (f, cmd) =>
            cmd.copy(input = ST_File(f))
          }
        },
      opt[File]('o', "output")
      ...
      opt[File]("linker")
        .action {
          cmdParseArticulacao { case (x, cmd) =>
            cmd copy (linkerPath = Some(x))
          }
        }
        .text(
...
end FECmdLineOptionParser


case object TFalhaNaRenderizacao
    extends ProblemType(3001, "Falha na renderização", PC_ErroGeralParser)

final case class ErroNaRenderizacao(ex: Exception)
    extends ParseProblem(
      TFalhaNaRenderizacao,
      Some(
        "Erro durante a renderização do documento em XML: %s" format ErroValidacaoSchema
          .unchain(ex)
          .mkString("", " => ", "")
      )
    )
...
object FECmdLine:
  ...
  private def setupLogging(log4jConfigFile: Option[File]): Unit =
    ...
    val cfgStream: InputStream = log4jConfigFile match {
      case None =>
        getClass.getClassLoader.getResourceAsStream("fecmdline.log4j.xml")
    ...
    parser.parse(args, CmdLineOpts()) foreach { opts =>
      ...
      opts.cmd match {
        case cmd: CmdParse =>
          ...
          val urnFragAutoridade =
            cmd.baseAutoridade.getOrElse(md0.urnFragAutoridade)
          val urnFragTipoNorma =
            cmd.baseTipoNorma.getOrElse(md0.urnFragTipoNorma)
          ...
          if verbose || cmd.showInfo then
            ...
            md.id foreach { id =>
              ...
              id.versao.foreach(v => {
                v.dataEvento foreach { d =>
                  println(s"    --data-evento ${d.urnRepr}")
                }
                println(s"    --tipo-evento ${v.evento}")
                v.timestamp foreach { d =>
                  println(s"    --timestamp-evento ${d.txt}")
                }
              })
            }
        ...
        case CmdHelp => parser.displayToErr(parser.usage)
  ...
  private def dumpProfile(p: DocumentProfile): Unit =
    ...
    println(
      s"   --prof-regex-epigrafe-continuacao '${regexEpigrafe.mkString("%")}'"
    )
    ...
    println(
      s"   --prof-regex-legislacao-citada '${regexLegislacaoCitada.mkString("%")}'"
    )
    println(s"   --prof-regex-local-data '${regexLocalData.mkString("%")}'")
    println(
      s"   --prof-regex-justificativa '${regexJustificativa.mkString("%")}'"
    )
  ...
  def process(
      ...
      mimeType: String,
      output: SinkType,
      linkerPath: Option[File],
      verbose: Boolean,
      errorOutput: ErrorOutput
  ): Unit =
    XHTMLProcessor.pipelineWithDefaultConverter(
      input.toByteArray,
      mimeType
    ) foreach { xhtml =>
      val blocks = Block fromNodes xhtml
      val (mpl1, falhasValidacao) =
        new ProjetoLeiParser(profile).fromBlocks(md, blocks)
      // Linker.system.terminate().wait() // Porque parar o linker aqui?
      val (res: Option[Elem], falhasXML: List[ParseProblem]) = mpl1 match {
        case None => (None, List())
        case Some(pl) =>
          try { val (xml, probs) = renderAndValidaXML(pl); (Some(xml), probs) }
          catch {
            case ex: Exception =>
              ex.printStackTrace()
  ...
  private def processArticulacao(cmd: CmdParseArticulacao, verbose: Boolean): Unit =
    ...
    val useLinker = cmd.linkerPath match {
      case Some(f) if f.canExecute =>
        ...
        if (verbose) {
          System.err.println(s"linker: using linkertool executable at $path")
        }
        sys.props += ("lexml.linkertool" -> path)
        true
      case _ =>
        if (verbose) {
          System.err.println("linker: skipping linker")
        }
        sys.props += ("lexml.skiplinker" -> "true")
        false
    }
