...
object HasId {
  ...
  private implicit class Unico(un : Boolean) {
    def unicoChar : String = if (un) { "u" } else { "" }
  }
  ...
  def renderId(r: Rotulo): String = r match {
    ...
    case RotuloSubCapitulo(_, _,_) => throw new IdRenderException("Sub-capítulo não suportado pelo parser")
        ...
        val res = rotulo match {
          case RotuloArtigo(_, _, _) =>
            List(
              ...
              Dispositivo(RotuloParagrafo(), conteudo, List(), List(), abreAspas = false, fechaAspas = fechaAspas, notaAlteracao))
        ...
        case (d1: Dispositivo, bl2) =>
          spanNivel(d1.rotulo.nivel, bl2) match {
            case (Nil, (a: Alteracao) :: bl4) =>
              agrupa(d1 copy (subDispositivos = d1.subDispositivos :+ (a copy (blocks = organizaDispositivos(reconheceOmissisVazio2(a.blocks), dentroAlteracao = true)))), bl4)
    ...
    def createPath(tail: List[Rotulo])(b: Block): Block = b match {
      case d: Dispositivo =>
        val path = d.rotulo match {
          case _: RotuloArtigo => d.rotulo :: tail.dropWhile(_.isInstanceOf[RotuloAgregador])
