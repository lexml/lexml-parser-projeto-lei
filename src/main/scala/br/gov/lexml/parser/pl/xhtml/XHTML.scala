...
enum XHTMLProcessorResult:
  case Failure extends XHTMLProcessorResult
  case Success(result: List[Node]) extends XHTMLProcessorResult
...
trait Converter:
  ...
  def deleteByPrefix(dir: File, prefix: String): Unit =
    dir
      .listFiles(new PrefixFileFilter(prefix): FileFilter)
...
final class DOCXConverter(otherConverter: Converter) extends Converter:
  override def convert(
    ...
    (srcExtension, dstExtension) match {
      case ("docx", "xhtml") =>
        DOCXReader
          .readDOCX(new ByteArrayInputStream(srcData))
          .get
          .toString
          .getBytes("utf-8")
...
final class AbiwordConverter(val removeTemporaryFiles: Boolean = true)
    extends Converter
    with Logging:
  ...
  override def convert(
    ...
    logger.info(
      s"abiword.convert: starting: srcExtension = $srcExtension, dstExtension = $dstExtension"
    )
    val srcFile = File.createTempFile("lexml-parser-pl", "." + srcExtension)
    logger.info(s"abiword.convert: srcFile = $srcFile")
    val baseName = srcFile.getName.substring(
      0,
      srcFile.getName.length - srcExtension.length - 1
    )
    val baseDir = srcFile.getParentFile
    val destFile = new File(baseDir, baseName + "." + dstExtension)
    logger.info(s"abiword.convert: destFile = $destFile")
    val (params, postProc) = dstExtension match {
      case "xhtml" =>
        (
          List(
            "--to=xhtml",
            "--exp-props=html4: no; declare-xml: yes; use-awml:no; embed-css: yes; embed-images: yes"
          ),
          TextUtils.fixXHTML(_: Array[Byte])
        )
      case "pdf" => (List("--to=pdf"), noPostProc _)
      case _ =>
        throw new RuntimeException(
          s"Abiword Converter does not support extension: $dstExtension"
        )
    }
    logger.info(s"abiword.convert: params = $params")
    try {
      FileUtils.writeByteArrayToFile(srcFile, srcData)
      val cmd: Array[String] =
        (("/usr/bin/abiword" :: params) :+ srcFile.getPath).toArray
      logger.info("running " + cmd.mkString(" "))
      val p =
        Runtime.getRuntime.exec(cmd, Array[String](), srcFile.getParentFile)
      logger.info("returned from abiword")
      p.waitFor
      postProc(FileUtils.readFileToByteArray(destFile))
    } finally {
      if removeTemporaryFiles then
        deleteByPrefix(baseDir, baseName)
      end if
    }
  end convert
end AbiwordConverter

@unused
final class OpenOfficeConverter(val removeTemporaryFiles: Boolean = true)
    extends Converter
    with Logging:
  ...
  override def convert(
    ...
    logger.info(
      "oo.convert: starting: srcExtension = " + srcExtension + ", dstExtension = " + dstExtension
    )
    ...
    try {
      FileUtils.writeByteArrayToFile(srcFile, srcData)
      val cmd: Array[String] =
        Array(pyodconverter, srcFile.getPath, destFile.getPath)
      val p =
        Runtime.getRuntime.exec(cmd, Array[String](), srcFile.getParentFile)
      p.waitFor
...
object XHTMLProcessor extends Logging:
  ...
  private val defaultConverter: Converter = new DOCXConverter(
    new AbiwordConverter
  )

  private def changeChildren[T <: Seq[Node]](f: Seq[Node] => Seq[Node]) =
    (e: T) => {
      e match {
        case Elem(pref, name, attrs, scope, children*) =>
          Elem(pref, name, attrs, scope, true, f(children)*).asInstanceOf[T]
        case _ => e
      }
    }
  ...
  private def xhtmlLoader(in: Array[Byte]): Elem = xhtmlLoader(
    new ByteArrayInputStream(in)
  )

  private def xhtmlLoader(is: InputStream): Elem =
    XhtmlParser(new BufferedSource(is)(Codec.UTF8))
      .collectFirst { case e: Elem => e }
      .get

  private def externalConvertToXhtml(extension: String) = (
    extension,
    (data: Array[Byte], converter: Converter) => {
  ...
  private val sourceProcessorMap: Map[String, (String, SourceProcessor)] = Map(
    (
      ...
      (
        "txt",
        (source: Array[Byte], _: Converter) => {
          val text = fixXHTML(source)

          val lines =
            scala.io.Source.fromBytes(text, "utf-8").getLines().to(List)
          @tailrec
          def toPars(
              l: List[String],
              r: List[String] = Nil,
            ...
            case Nil =>
              s match { case Nil => r; case _ => s.mkString("", " ", "") :: r }
            case x :: xs if x.trim.isEmpty =>
              toPars(xs, s.mkString("", " ", "") :: r)
    ...
    (
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      externalConvertToXhtml("docx")
    )
  )

  private def convertSrcToXHTML(
      source: Array[Byte],
      mimeType: String,
      converter: Converter
  ): Option[Elem] =
  ...
  private def selectBaseElems(root: Elem): List[Elem] =
    ...
    def getIdOrType(n: Node) =
      getAttr(n, "id").orElse(getAttr(n, "type")).getOrElse("")
    val childs = trim(belems)
    val childs1 = childs.filter((n: Node) => {
      val x = getIdOrType(n); x != "header" && x != "footer"
    })
    val (cl1, cl2) = childs1.span({
      case e: Elem => e.label == "table"; case _ => false
    })
    val childs3 = (cl1 \\ "*").filter {
      case e: Elem =>
        e.label == "p" || e.label == "h1" || e.label == "h2" || e.label == "h3"
      case _ => false
    } ++ cl2
  ...
  private val parLabels: Set[String] =
    Set("p", "h1", "h2", "h3", "h4", "blockquote")

  private val isValidElem: PartialFunction[Node, Node] = {
    case e @ Elem(_, "table", _, _, _*) => e
    case Elem(pref, "ol", _, scope, children*) =>
      Elem(pref, "ol", Null, scope, true, children.collect(isValidElem)*)
    case Elem(pref, "li", _, scope, children*) =>
      Elem(
        pref,
        "li",
        Null,
        scope,
        true,
        children.collect(isValidElem orElse isContent)*
      )
    case Elem(pref, label, attrs, scope, children*)
        if parLabels.contains(label) =>
      Elem(pref, "p", attrs, scope, true, children*)
  }
  ...
  private def wrapText(nl: List[Node]): List[Node] =
    ...
    def docollect(
      ...
      case (e: Elem) :: r if !blockElems.contains(e.label) =>
        docollect(r, e :: accum, accum2)
      case (e: Elem) :: r if trim(accum).isEmpty =>
        docollect(r, Nil, e :: accum2)
      case (e: Elem) :: r =>
        docollect(r, Nil, e :: <p>{NodeSeq fromSeq accum.reverse}</p> :: accum2)
    }
    docollect(nl, Nil)
  end wrapText

  private def trimLeft(nl: List[Node]) = nl.dropWhile {
    case t: Text => t.text.trim.isEmpty; case _ => false
  }
  ...
  private def explodeDivs(divs: List[Elem]) =
    def explode(n: Node): List[Node] = n match {
      case e: Elem if explodedBlockElements.contains(e.label) =>
        wrapText(e.child.toList).flatMap(explode)
      case e: Elem if explodedInlineElements.contains(e.label) =>
        e.child.toList.flatMap(explode)
      case e: Elem if e.label == "td" =>
        trim(e.child.toList) match {
          case List(e2: Elem) if e2.label == "p" =>
            List(e copy (child = e2.child.toList.flatMap(explode)))
  ...
  private def mapToAttributes(m: Map[String, String]) =
    m.foldRight(Null.asInstanceOf[MetaData])((kv, md) => {
      val (k, v) = kv
  ...
  private def cleanAttributes: Seq[Node] => Seq[Node] = bottomUp { (n: Node) =>
        ...
        e match {
          case Elem(pref, name, attrs, scope, children*) =>
            Elem(
              pref,
              name,
              mapToAttributes(f(attrs.asAttrMap)),
              scope,
              true,
              children*
            )
    ...
    val filterRedundantTDAttrs = (m: Map[String, String]) =>
      m.filter {
    ...
    def keepOnly(keyNames: String*) =
      (m: Map[String, String]) => m.filter(x => keyNames.contains(x._1))
    ...
    n match {
      case Elem(_, label, _, _, _*) =>
        ...
        label match {
          ...
          case "td" =>
            changeAttrs(
              filterRedundantTDAttrs.andThen(keepOnly("colspan", "rowspan"))
            )(e)
  ...
  private def fixSpans(nl: List[Node]): List[Node] =
    nl.flatMap({
      case e @ Elem(pref, label, attrs, scope, child*) =>
        val child2 = fixSpans(child.toList)
        e.label match {
          case "span" => makeSpanOrIandB(pref, scope, attrs, child2)
          case _      => List(Elem(pref, label, attrs, scope, true, child2*))
        }
      case n => List(n)
    })


  private def makeSpanOrIandB(
      prefix: String,
    ...
    val otherStyles =
      styles - "font-style" - "font-weight" - "text-decoration" - "vertical-align"
    val restMap: Map[String, String] =
      if otherStyles.isEmpty then otherAttrs
      else
        otherAttrs + ((
          "style",
          otherStyles.toList.map(x => x._1 + ":" + x._2).mkString("", ";", "")
        ))
    var e =
      if restMap.isEmpty then child
      else
        Elem(prefix, "span", mapToAttributes(restMap), scope, true, child*)
    e = if italicPresent  then Elem(prefix, "i", Null, scope, true, e*) else e
    e = if boldPresent    then Elem(prefix, "b", Null, scope, true, e*) else e
    e = if isSuperScript  then Elem(prefix, "sup", Null, scope, true, e*) else e
    e = if isSubScript    then Elem(prefix, "sub", Null, scope, true, e*) else e
    if hasUnderline then logger.warn("text has underline!")
    e
  ...
  private val cleanSeqNodes: List[Node] => List[Node] = bottomUp(
    mapElements(
      id,
      (e: Elem) =>
        if (validElements.contains(e.label)) { e }
        else { e.child }
    )
  )
  ...
  private val renameHeadings: List[Node] => List[Node] = bottomUp(
    mapElements(
      id,
      (e: Elem) =>
        if (headings.contains(e.label)) { e copy (label = "p") }
        else { e }
    )
  )

  private def bottomUp(f: Node => Seq[Node]): Seq[Node] => List[Node] =
    (ns: Seq[Node]) => {
  ...
  private def topDown(f: Node => Seq[Node]): Seq[Node] => List[Node] =
    (ns: Seq[Node]) => {
      val chChildren = (n: Node) => changeChildren(bottomUp(f))(n)
      val nl = ns.iterator.toList
  ...
  def topDownUntil(
      f: PartialFunction[Node, Seq[Node]]
  ): Seq[Node] => List[Node] = (ns: Seq[Node]) => {
  ...
  private def transformTextWith[A](
      f: (A, String) => (String, A)
  )(a: A): Seq[Node] => List[Node] = (ns: Seq[Node]) => {
    def doit(bl: (A, List[Node]), n: Node): (A, List[Node]) = {
      ...
      n match {
        ...
        case Elem(pref, name, attrs, scope, children*) =>
          ...
          (b2, Elem(pref, name, attrs, scope, true, rl.reverse*) :: l)
        case _ => (b1, n :: l)
      }
    }
  ...
  private def transformTextBackwardsWith[A](
      f: (A, String) => Option[(String, A)]
  )(a: A): Seq[Node] => List[Node] =
    (ns: Seq[Node]) => {
      def doit(
          bl: (A, List[Node], Boolean),
          n: Node
      ): (A, List[Node], Boolean) = {
        val (b1, l, skip) = bl
        if (skip) { (b1, n :: l, skip) }
          ...
          n match {
            ...
            case Elem(pref, name, attrs, scope, children*) =>
              val (b2, rl, skip2) =
                children.reverse.foldLeft(b1, List[Node](), skip)(doit)
              (b2, Elem(pref, name, attrs, scope, true, rl*) :: l, skip2)
            case _ => (b1, n :: l, skip)
          }
      }
  ...
  private val normalizeSpace: Seq[Node] => List[Node] =
    topDownUntil((n: Node) =>
      n match {
        case Elem(_, label, _, _, _*)
            if label == "p" || label == "li" || label == "blockquote" =>
          val ns1 = transformTextWith(cleanSpaces)(true)(n)
          val ns2 = transformTextBackwardsWith((skip: Boolean, t: String) =>
            if (skip) { None }
            else { Some(re5.replaceFirstIn(t, ""), true) }
          )(false)(ns1)
          ns2
      }
    )
  ...
  private val cleanSpuriousSpans = topDown {
    case n @ Elem(_, "span", attrs, _, children*)
        if getAttr(attrs, "style").isEmpty ||
          n.text.trim.isEmpty =>
      children
  ...
  private val cleanNameSpaces = topDown((n: Node) =>
    n match {
      case Elem(pref, label, attrs, _, cl*) =>
        Elem(pref, label, attrs, TopScope, true, cl*)
      case _ => n
    }
  )
  ...
  private def cleanRepeatedEmptyParagraphs(ns: Seq[Node]) =
    def f(n: Node, ns: List[Node]) =
      n match {
        case Elem(_, name, _, _, _*) if name == "p" || name == "blockquote" =>
          ns match {
            case (n2 @ Elem(_, "p", _, _, _*)) :: _ =>
              if n.text.isEmpty && n2.text.isEmpty then ns else n :: ns
            case _ => n :: ns
          }
        case _ => n :: ns
      }
      ...
      id, {
        case Elem(pref, label, attrs, scope, cl*) =>
          def mergeit(n: Node, ns: List[Node]) = {
            ...
            val res = n match {
              case Elem(_, "span", attrs1, _, cl1*) =>
                nss match {
                  case Elem(pref, "span", attrs2, scope, cl2*) :: ns2 =>
          ...
          Elem(pref, label, attrs, scope, true, cll*)
      }
    )
  )
  ...
  def pipelineWithDefaultConverter(
      source: Array[Byte],
      mimeType: String
  ): Option[List[Node]] =
  ...
  ): XHTMLProcessorResult =
    pipeline(IOUtils.toByteArray(rtfSource), "text/rtf", converter) match {
      case None    => XHTMLProcessorResult.Failure
      case Some(x) => XHTMLProcessorResult.Success(x)
    }
end XHTMLProcessor
