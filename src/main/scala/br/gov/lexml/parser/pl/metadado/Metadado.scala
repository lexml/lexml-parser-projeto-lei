...
object Fixer {
	...
	def replace(in : CharSequence, repMap : Map[String,String]): String = regex.replaceAllIn(in, m => repMap(m.matched))
}
...
case class Timestamp(ano : Int, mes : Int, dia : Int, hora : Int, minuto : Int) {
	lazy val txt: String = "%04d-%02d-%02dt%02d.%02d".format(ano,mes,dia,hora,minuto)
}
...
case class Versao(dataEvento : Option[Data] = None, evento : String = "leitura", timestamp : Option[Timestamp]) {
	...
	lazy val metadadoRepr: String = dataEvento.map(_.urnRepr).getOrElse("data.evento") + ";" + evento + timestamp.map(s => ";" + s.txt).getOrElse("")
}
...
case class Id(num : Int = 1, complemento : Option[Int] = None, anoOuData : Either[Int,Data] = Right(Data.today),  
    ...
    lazy val anoOuDataUrn: String = anoOuData match {
  ...
  def changeVersao(f : Option[Versao] => Option[Versao]): Id = copy(versao = f(versao))
  lazy val numeroComplementoRepr: String = Metadado.renderNumero(num) + complemento.map(Metadado.renderComplemento).getOrElse("")
}

object Id {
	private val re = """(\d\d\d\d)(?:-(\d\d)-(\d\d))?;(\d+)(?:-(\d+))?(?:@(data\.evento|(\d\d\d\d)-(\d\d)-(\d\d));([a-z.]+);(\d\d\d\d)-(\d\d)-(\d\d)t(\d\d)\.(\d\d))?""".r
...
case class Metadado(profile : DocumentProfile, localidade : Option[String] = None, autoridade : Option[String] = None, tipoNorma : Option[String] = None, id : Option[Id] = None, hashFonte : Option[Array[Byte]]) {
  lazy val urnFragAutoridade: String = autoridade.getOrElse(profile.urnFragAutoridade)
  lazy val urnFragTipoNorma: String = tipoNorma.getOrElse(profile.urnFragTipoNorma)
  lazy val urnFragLocalidade: String = localidade.orElse(profile.urnFragLocalidade).getOrElse("br")
  ...
  lazy val urnContextoLinker: String = if(id.isDefined) { urn } else { this.copy(id = Some(Id())).urn }
...
object Metadado {
  lazy val pt_BR: Locale = Locale.getAvailableLocales.view.filter(_.getCountry == "BR").head
